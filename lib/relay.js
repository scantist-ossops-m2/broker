        ...
        if (streamingID) {
          ...
          request(req)
            .on('response', (response) => {
              console.log("Response received - setting up stream to Server");
              ...
              // });
              buffer.pipe(request({
                url: `http://localhost:7341/broker-data/${brokerToken}/${streamingID}`,
                method: 'post',
                headers: {
              ...
              console.log("Pipe set up");
              const ioData = JSON.stringify({
                status,
                headers: response.headers
              });
              const ioDataLength = ioData.length;
              console.log(`ioData has length of ${ioDataLength}`);
            ...
            .on('data', (chunk) => {
              // console.log("writing data to buffer");
            ...
            .on('end', () => {
              console.log("writing end to buffer");
              buffer.end();
            })
            /*.on('data', (chunk) => {
              if (config.RES_BODY_URL_SUB && isResponseJson) {
