...
function requestHandler(filterRules) {
  ...
  return (req, res) => {
    ...
    filters(req, (error, result) => {
      ...
      if (result.stream || (res.locals && res.locals.capabilities && res.locals.capabilities.includes('post-streams'))) {
        ...
        if (streamingID) {
          ...
          try {
            ...
            if (io.socketVersion === 1) {
              ...
              buffer.on('error', e => logger.error('received sending data to buffer', e))
              rqst
                ...
                .on('response', (response) => {
                  ...
                  buffer.pipe(request({
                    url: `${config.brokerServerUrl}/broker-data/${brokerToken}/${streamingID}`,
                    method: 'post',
                    headers: {
                  ...
                  }).on('error', e => logger.error('received error sending data via POST', e)).on('response', (r => {
                    if (r.statusCode !== 200) {
                      logger.error('Received unexpected response uploading data', r.statusCode, r.statusMessage, r.headers, r.body?.toString());
                    }
                  })));
                  ...
                  const intArray = new Uint8Array(4);
                  intArray[0] = ioDataLength & 0xFF;
                  intArray[1] = (ioDataLength >> 8) & 0xFF;
                  intArray[2] = (ioDataLength >> 16) & 0xFF;
                  intArray[3] = (ioDataLength >> 24) & 0xFF;
                  buffer.cork();
                  buffer.write(intArray);
                  buffer.write(ioData);
                  buffer.uncork();
                })
                .on('data', (chunk) => {
                  logger.trace("writing data to buffer");
