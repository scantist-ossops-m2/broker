...
module.exports = ({ config = {}, port = null, filters = {} }) => {
  ...
  wss.on('connection', (socket, req) => {
    ...
    socket.on('message', data => {
      try {
        ...
        //     logger.debug("Handling response-data request - io bits", {json: JSON.parse(ioJson)});
        //     streamHandler(streamingId, null, false, JSON.parse(ioJson), null);
        //   } else {
        ...
        if (bytesRead < data.length) {
          logger.trace("Handling response-data request - data part");
  ...
  app.post(
    ...
    (req, res, next) => {
      ...
      const maskedToken = maskToken(token);
      logger.info("Handling response-data request", {maskedToken, streamingId});
      req.maskedToken = maskedToken;
      ...
      req.on('data', function (data) {
        ...
        logger.debug("Handling response-data request - end part");
        streamHandler(streamingId, null, true, null, null);
        res.status(200).json({});
      }).on('error', err => {
